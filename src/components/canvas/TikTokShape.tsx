...
export type TikTokShape = TLBaseShape<
  "tiktok",
  {
    w: number;
    h: number;
...
export class TikTokShapeUtil extends BaseBoxShapeUtil<TikTokShape> {
  static override type = "tiktok" as const;

  static override props: RecordProps<TikTokShape> = {
    w: T.number,
    h: T.number,
    url: T.string,
  };

  getDefaultProps(): TikTokShape["props"] {
    return { w: 340, h: 480, url: "" };
  }

  override canResize() { return true; }
  override canBind() { return true; }

  override onResize(shape: TikTokShape, info: TLResizeInfo<TikTokShape>) {
    return resizeBox(shape, info);
  }

  component(shape: TikTokShape) {
    return <TikTokComponent shape={shape} />;
  }

  indicator(shape: TikTokShape) {
    return <rect width={shape.props.w} height={shape.props.h} rx={12} ry={12} />;
  }
}

function TikTokComponent({ shape }: { shape: TikTokShape }) {
  const editor = useEditor();
  const [showUrlInput, setShowUrlInput] = useState(!shape.props.url);
  const [urlValue, setUrlValue] = useState(shape.props.url || "");
  const videoId = getTikTokVideoId(shape.props.url);

  const handleSubmit = useCallback(() => {
    editor.updateShape({ id: shape.id, type: "tiktok", props: { url: urlValue.trim() } });
  ...
  if (!videoId || showUrlInput) {
    return (
      <HTMLContainer style={{
        ...
        <SourceDot shapeId={shape.id} shapeType="tiktok" />
